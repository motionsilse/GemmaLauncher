...
public sealed class LauncherViewModel : ObservableObject, IAsyncDisposable
{
    ...
    public ModelDefinition? SelectedModel
    {
        ...
        set
        {
            if (value is null || !CanSelectModel || !Set(ref _selectedModel, value)) return;
            _settings.SelectedModelId = value.Id;
            _installed = null;
            _error = "";
            _rebuildingChoices = true;
            ContextChoices.Clear();
            foreach (var size in value.Profile.ContextSizes)
                ContextChoices.Add(new(size, size == value.Profile.DefaultContext));
            var selected = _settings.Contexts.GetValueOrDefault(value.Id, value.Profile.DefaultContext);
            SelectedContext = ContextChoices.FirstOrDefault(c => c.Value == selected) ?? ContextChoices.First(c => c.Value == value.Profile.DefaultContext);
    ...
    private IInstallationService CreateInstaller() => new InstallationService(_paths, searchDirectories: _settings.ModelFolders.ToArray());
    ...
    {
        foreach (var property in new[] { nameof(SelectedPresentation), nameof(IsBusy), nameof(CanSelectModel), nameof(CanChangeContext), nameof(CanOpenWebUi), nameof(HasPendingContext), nameof(IsProgressVisible), nameof(IsIndeterminate), nameof(ProgressPercent), nameof(ProgressMessage), nameof(StatusNote), nameof(MemoryUsage), nameof(MemoryUsageLabel), nameof(DownloadLabel), nameof(ApiModelId), nameof(ConnectionLabel), nameof(ContextHint), nameof(StatusLabel), nameof(StatusTitle), nameof(StatusDescription), nameof(PrimaryLabel), nameof(IsServerRunning) }) OnPropertyChanged(property);
    ...
    private void BeginOperation(Func<CancellationToken, Task> action)
    {
        ...
        Refresh();
        _operation = RunOperationAsync(action, _operationCancellation);
    }

    private async Task RunOperationAsync(Func<CancellationToken, Task> action, CancellationTokenSource cancellation)
    {
        try { await action(cancellation.Token); }
        catch (MissingRuntimePrerequisiteException ex) { _error = ex.Message; _prerequisiteDownload = ex.DownloadUri; }
        catch (OperationCanceledException) { await StopAfterFailureAsync(); Note(T("ui.note.cancelled")); }
        catch (Exception ex) { await StopAfterFailureAsync(); _error = ex.Message; SaveError(ex); }
    ...
    private void SaveSettings() { try { _store.Save(_settings); } catch (Exception ex) { Note(T("ui.note.saveFailed", ex.Message)); } }
